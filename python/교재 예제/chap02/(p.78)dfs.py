...
#  for elem in open_stack:
#        print(elem)
#  print("END OF OPENQ")
  
while len(open_stack) != 0: 
  current = open_stack.pop()			# OPEN 리스트의 끝에서 삭제
  ...
  for state in current.expand(moves):
      if (state in closed_stack) or (state in open_stack):	# 이미 거쳐간 노드이면
          continue				# 노드를 버린다.      
      else: 
          open_stack.append(state)		# OPEN 리스트의 끝에 추가