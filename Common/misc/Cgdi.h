...
{
	RGB(255,0,0), RGB(0,0,255), RGB(0,255,0), RGB(0,0,0), RGB(255,200,200), RGB(200,200,200),
	RGB(255,255,0), RGB(255,170,0), RGB(255,0,170), RGB(133,90,0), RGB(255,255,255),
...
public:
	...
	enum
	{
		red, blue, green, black, pink, grey,
		yellow, orange, purple, brown, white, dark_green,
		light_blue, light_grey, light_pink, hollow
	};
...
public:
	...
	{
		TextOut(m_hdc, x, y, s.c_str(), static_cast<int>(s.size()));
	}

	void TextAtPos(double x, double y, const std::string& s)
	{
		TextOut(m_hdc, static_cast<int>(x), static_cast<int>(y), s.c_str(), static_cast<int>(s.size()));
	}

	void TextAtPos(Vector2D pos, const std::string& s)
	{
		TextOut(m_hdc, static_cast<int>(pos.x), static_cast<int>(pos.y), s.c_str(), static_cast<int>(s.size()));
	}
	...
	//----------------------------pixels
	void DrawDot(Vector2D pos, COLORREF color) { SetPixel(m_hdc, static_cast<int>(pos.x), static_cast<int>(pos.y), color); }
	...
	{
		MoveToEx(m_hdc, static_cast<int>(a), static_cast<int>(b), nullptr);
		LineTo(m_hdc, static_cast<int>(x), static_cast<int>(y));
	}
	...
	void PolyLine(const std::vector<Vector2D>& points)
	{
		...
		MoveToEx(m_hdc, static_cast<int>(points[0].x), static_cast<int>(points[0].y), nullptr);

		for (unsigned int p = 1; p < points.size(); ++p)
		{
			LineTo(m_hdc, static_cast<int>(points[p].x), static_cast<int>(points[p].y));
		}
	}
	...
	void LineWithArrow(Vector2D from, Vector2D to, double size)
	{
		...
		//draw the line
		MoveToEx(m_hdc, static_cast<int>(from.x), static_cast<int>(from.y), nullptr);
		LineTo(m_hdc, static_cast<int>(CrossingPoint.x), static_cast<int>(CrossingPoint.y));
	...
	{
		Line(static_cast<int>(pos.x) - diameter, static_cast<int>(pos.y) - diameter, static_cast<int>(pos.x) + diameter, static_cast<int>(pos.y) + diameter);
		Line(static_cast<int>(pos.x) - diameter, static_cast<int>(pos.y) + diameter, static_cast<int>(pos.x) + diameter, static_cast<int>(pos.y) - diameter);
	}

	//---------------------Geometry drawing methods
	void Rect(int left, int top, int right, int bot) { Rectangle(m_hdc, left, top, right, bot); }
	void Rect(double left, double top, double right, double bot) { Rectangle(m_hdc, static_cast<int>(left), static_cast<int>(top), static_cast<int>(right), static_cast<int>(bot)); }
	...
	void ClosedShape(const std::vector<Vector2D>& points)
	{
		...
		LineTo(m_hdc, static_cast<int>(points[0].x), static_cast<int>(points[0].y));
	}


	void Circle(Vector2D pos, double radius)
	{
		Ellipse(m_hdc, static_cast<int>((pos.x - radius)), static_cast<int>((pos.y - radius)), static_cast<int>((pos.x + radius + 1)), static_cast<int>((pos.y + radius + 1)));
	}

	void Circle(double x, double y, double radius)
	{
		Ellipse(m_hdc, static_cast<int>((x - radius)), static_cast<int>((y - radius)), static_cast<int>((x + radius + 1)), static_cast<int>((y + radius + 1)));
	}
