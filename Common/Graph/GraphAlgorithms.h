...
public:
	Graph_SearchDFS(const graph_type& graph, int source, int target = -1) : m_Graph(graph), m_iSource(source), m_iTarget(target),
		m_bFound(false), m_Visited(m_Graph.NumNodes(), unvisited), m_Route(m_Graph.NumNodes(), no_parent_assigned)
	{
		m_bFound = Search();
	}
...
std::list<int> Graph_SearchDFS<graph_type>::GetPathToTarget() const
{
	...
	//just return an empty path if no path to target found or
	//if no target has been specified
...
private:
	...
	//this holds the route taken to the target. Given a node index, the value
	//at that index is the node's parent. ie if the path to the target is 3-8-27
	//then m_Route[8] will hold 3 and m_Route[27] will hold 8.
	std::vector<int>  m_Route;
	...
	//As the search progresses, this will hold all the edges the algorithm has
	//examined. THIS IS NOT NECESSARY FOR THE SEARCH, IT IS HERE PURELY TO PROVIDE THE USER WITH SOME VISUAL FEEDBACK
	std::vector<const Edge*>  m_SpanningTree;
...
public:
	Graph_SearchBFS(const graph_type& graph, int source, int target = -1) : m_Graph(graph), m_iSource(source), m_iTarget(target),
		m_bFound(false), m_Visited(m_Graph.NumNodes(), unvisited), m_Route(m_Graph.NumNodes(), no_parent_assigned)
	{
		m_bFound = Search();
	}
...
private:
	...
	//this method performs the IDS search
	bool Search();

public:
	Graph_SearchIDS(const graph_type& graph, int source, int target = -1) : m_Graph(graph), m_iSource(source), m_iTarget(target),
		m_bFound(false), m_Visited(m_Graph.NumNodes(), unvisited), m_Route(m_Graph.NumNodes(), no_parent_assigned)
	{
		m_bFound = Search();
	}
...
public:
	Graph_SearchDijkstra(const graph_type& graph, int source, int target = -1) : m_Graph(graph), m_ShortestPathTree(graph.NumNodes()),
		m_SearchFrontier(graph.NumNodes()), m_CostToThisNode(graph.NumNodes()), m_iSource(source), m_iTarget(target)
	{
		Search();
	}
...
{
	//create an indexed priority queue that sorts smallest to largest(front to back).
	//Note that the maximum number of elements the iPQ may contain is N.
	//This is because no node can be represented on the queue more than once.
	...
	while (!pq.empty())
	{
		//get lowest cost node from the queue. Don't forget, the return value is a *node index*, not the node itself.
		//This node is the node not already on the SPT that is the closest to the source node
...
private:
	...
	//the heuristic cost from n to the target node. This is the vector the iPQ indexes into.
...
public:
	Graph_SearchAStar(graph_type& graph, int source, int target) : m_Graph(graph), m_ShortestPathTree(graph.NumNodes()), m_SearchFrontier(graph.NumNodes()),
		m_GCosts(graph.NumNodes(), 0.0), m_FCosts(graph.NumNodes(), 0.0), m_iSource(source), m_iTarget(target)
	{
		Search();
	}
...
void Graph_SearchAStar<graph_type, heuristic>::Search()
{
	...
	//The nodes with the lowest overall F cost (G+H) are positioned at the front.
	...
	while (!pq.empty())
	{
		...
		for (const Edge* pE = ConstEdgeItr.begin(); !ConstEdgeItr.end(); pE = ConstEdgeItr.next())
		{
			...
			//if this node is already on the frontier but the cost to get here is cheaper
			//than has been found previously, update the node costs and frontier accordingly.
...
public:
	Graph_MinSpanningTree(graph_type& G, int source = -1) : m_Graph(G), m_SpanningTree(G.NumNodes()), m_Fringe(G.NumNodes()), m_CostToThisNode(G.NumNodes(), -1)
	{
		if (source < 0)
		{
...
template <class graph_type>
void Graph_MinSpanningTree<graph_type>::Search(const int source)
{
