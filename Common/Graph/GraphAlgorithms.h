...
{
	using EdgeInfo = std::pair<const Edge*, int>;

	//create a std stack of edges
	std::stack<EdgeInfo> stack;
	...
	stack.push(std::make_pair(&Dummy, present_depth));

	while (true)
	{
		if (stack.empty())
		{
			std::for_each(m_Visited.begin(), m_Visited.end(), [](int& n) { return n = unvisited; });
			std::for_each(m_Route.begin(), m_Route.end(), [](int& n) { return n = no_parent_assigned; });
		...
		//grab the next edge
		//Next.first == Edge
		//Next.second == depth of Edge
		EdgeInfo Next = stack.top();

		//remove the edge from the stack
		stack.pop();

		// present_depth¸¦ º¯°æ
		present_depth = Next.second;

		if (m_Visited[Next.first->To()] == visited)
		{
			continue;
		}
		...
		if (Next.second < depth_limit)
		{
			...
			for (const Edge* pE = ConstEdgeItr.begin(); !ConstEdgeItr.end(); pE = ConstEdgeItr.next())
			{
				if (m_Visited[pE->To()] == unvisited)
				{
					stack.push(std::make_pair(pE, present_depth));
				}
			}
		}
	}
