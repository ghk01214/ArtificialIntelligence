...
void GraphHelper_CreateGrid(graph_type& graph, int cySize, int cxSize, int NumCellsY, int NumCellsX)
{
	...
	double CellWidth = static_cast<double>(cySize) / static_cast<double>(NumCellsX);
	double CellHeight = static_cast<double>(cxSize) / static_cast<double>(NumCellsY);
...
double CalculateAverageGraphEdgeLength(const graph_type& G)
{
	...
	return TotalLength / static_cast<double>(NumEdgesCounted);
}
