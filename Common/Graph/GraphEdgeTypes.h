...
public:
	...
	enum
	{
		normal = 0, swim = 1 << 0, crawl = 1 << 1, creep = 1 << 3,
		jump = 1 << 3, fly = 1 << 4, grapple = 1 << 5, goes_through_door = 1 << 6
	};
...
public:
	NavGraphEdge(int from, int to, double cost, int flags = 0, int id = -1) : GraphEdge(from, to, cost), m_iFlags(flags), m_iIDofIntersectingEntity(id) {}
	...
	{
		os << "m_iFrom: " << e.m_iFrom << " m_iTo: " << e.m_iTo << " m_dCost: " << e.m_dCost << " m_iFlags: " << e.m_iFlags << " ID: " << e.m_iIDofIntersectingEntity << std::endl;
		return os;
	}
};