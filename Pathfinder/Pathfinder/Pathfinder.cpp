...
void Pathfinder::CreateGraph(int CellsUp, int CellsAcross)
{
	...
	m_iCellsY = CellsUp;
	m_dCellWidth = static_cast<double>(m_icxClient) / static_cast<double>(CellsAcross);
	m_dCellHeight = static_cast<double>(m_icyClient) / static_cast<double>(CellsUp);
...
bool Pathfinder::PointToIndex(POINTS p, int& NodeIndex)
{
	...
	int x = static_cast<int>((static_cast<double>((p.x) / m_dCellWidth)));
	int y = static_cast<int>((static_cast<double>((p.y) / m_dCellHeight)));
...
void Pathfinder::Save(char* FileName)
{
	...
	save << m_iCellsX << std::endl;
	save << m_iCellsY << std::endl;

	//save the terrain
	for (unsigned int t = 0; t < m_TerrainType.size(); ++t)
	{
		if (t == m_iSourceCell)
			save << source << std::endl;
		else if (t == m_iTargetCell)
			save << target << std::endl;
		else
			save << m_TerrainType[t] << std::endl;
	}
}
...
void Pathfinder::Render()
{
	...
	{
		int left = static_cast<int>((m_pGraph->GetNode(nd).Pos().x - m_dCellWidth / 2.0));
		int top = static_cast<int>((m_pGraph->GetNode(nd).Pos().y - m_dCellHeight / 2.0));
		int right = static_cast<int>((1 + m_pGraph->GetNode(nd).Pos().x + m_dCellWidth / 2.0));
		int bottom = static_cast<int>((1 + m_pGraph->GetNode(nd).Pos().y + m_dCellHeight / 2.0));
		...
		{
			gdi->ThickBlackPen();
			gdi->Cross(Vector2D(m_pGraph->GetNode(nd).Pos().x - 1, m_pGraph->GetNode(nd).Pos().y - 1), static_cast<int>(((m_dCellWidth * 0.6) / 2.0)));
		}
	...
	if (m_dTimeTaken)
	{
		...
		std::string time = ttos(m_dTimeTaken, 8);
		std::string s = "Time Elapsed for " + GetNameOfCurrentSearchAlgorithm() + " is " + time;
